...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const answer = await app_js_1.prisma.answer.findUnique({
            where: { id },
            include: {
                user: {
...
router.patch('/:id', auth_js_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const answer = await app_js_1.prisma.answer.findUnique({ where: { id } });
        if (!answer) {
            return res.status(404).json({ error: 'Answer not found' });
        }
        ...
        const updated = await app_js_1.prisma.answer.update({
            where: { id },
            data,
        });
...
router.delete('/:id', auth_js_1.authenticate, async (req, res) => {
    try {
        ...
        await app_js_1.prisma.answer.delete({ where: { id } });
...
router.post('/:id/accept', auth_js_1.authenticate, async (req, res) => {
    try {
        ...
        await app_js_1.prisma.answer.updateMany({
            where: {
