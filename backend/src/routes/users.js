...
router.post('/:id/enroll', auth_js_1.authenticate, auth_js_1.requireCreatorOrAdmin, async (req, res) => {
    try {
        const userId = req.params.id;
        const { courseId } = req.body;
        if (!courseId)
            return res.status(400).json({ error: 'courseId is required' });
        const user = await (0, db_js_1.queryOne)('SELECT id, name, email, role FROM users WHERE id = ?', [userId]);
        if (!user)
            return res.status(404).json({ error: 'User not found' });
        ...
        const course = await (0, db_js_1.queryOne)('SELECT id, title, status, creatorId FROM courses WHERE id = ?', [courseId]);
        if (!course)
