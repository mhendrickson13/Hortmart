...
router.patch('/', auth_js_1.authenticate, requireAdmin, async (req, res) => {
    try {
        ...
        for (const [key, value] of entries) {
            // Validate known keys
            if (!['webhookUrl', 'platformLanguage'].includes(key))
                continue;
...
router.post('/regenerate-og', auth_js_1.authenticate, requireAdmin, async (req, res) => {
    try {
        const { upsertCourseOgPage } = await import('../og.js');
        const courses = await (0, db_js_1.query)('SELECT id, title, subtitle, description, coverImage FROM courses WHERE status = ?', ['PUBLISHED']);
        let ok = 0;
        for (const c of courses) {
            try {
                await upsertCourseOgPage(c);
                ok++;
            }
            catch { /* skip */ }
        }
