...
router.post('/:id/accept', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!canAccept) {
      const lesson = await queryOne<any>('SELECT moduleId FROM lessons WHERE id = ?', [question.lessonId]);
      const mod = lesson ? await queryOne<any>('SELECT courseId FROM modules WHERE id = ?', [lesson.moduleId]) : null;
      const course = mod ? await queryOne<any>('SELECT creatorId FROM courses WHERE id = ?', [mod.courseId]) : null;
      if (course && course.creatorId === req.user!.id) canAccept = true;
    }
    if (!canAccept) {
