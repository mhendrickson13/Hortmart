...
async function getCreatorStats(creatorId: string) {
  ...
  const publishedCourses = courses.filter((c: CourseType) => c.status === "PUBLISHED").length;
  const totalEnrollments = courses.reduce((sum: number, c: CourseType) => sum + c._count.enrollments, 0);
  const totalRevenue = courses.reduce((sum: number, c: CourseType) => sum + c.price * c._count.enrollments, 0);
  const totalReviews = courses.reduce((sum: number, c: CourseType) => sum + c._count.reviews, 0);
  
  const allRatings = courses.flatMap((c: CourseType) => c.reviews.map((r: { rating: number }) => r.rating));
...
async function getLearnerStats(userId: string) {
  ...
  type LessonType = EnrollmentType['course']['modules'][0]['lessons'][0];
  type ProgressType = EnrollmentType['lessonProgress'][0];
  ...
  enrollments.forEach((enrollment: EnrollmentType) => {
    const allLessons = enrollment.course.modules.flatMap((m) => m.lessons);
    const completedLessons = enrollment.lessonProgress.filter(
      (p: ProgressType) => p.completedAt !== null
    ).length;
    ...
    enrollment.lessonProgress.forEach((progress: ProgressType) => {
      const lesson = allLessons.find((l: LessonType) => l.id === progress.lessonId);
      if (lesson) {
        totalWatchTimeSeconds += (lesson.durationSeconds * progress.progressPercent) / 100;
      }
    });
  });

  const totalWatchHours = Math.round(totalWatchTimeSeconds / 3600);
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-caption text-text-3">{progress}%</span>
                        </div>
                      </div>
                    </Link>
                  );
